...
class LinuxV4L2CaptureFrameProbe:
    ...
    def __init__(
        ...
        *,
        ...
        self._poll_factory, self._reader, self._close = poll_factory, reader, closer
    ...
    def _opened(self, fd: int, deadline: float) -> CaptureFrameProbeResult:
        ...
        try:
            try:
                if not stat.S_ISCHR(self._fstat(fd).st_mode):
                    reason = "not_character_device"
                else:
                    cap = bytearray(_CAPABILITY_SIZE)
                    reason, cap_ok = self._ioctl_with_retry(
                        fd, u.VIDIOC_QUERYCAP, cap, deadline, "capability"
                    )
                    if reason is None:
                    ...
                    if reason is None:
                        ...
                        reason, fmt_ok = self._ioctl_with_retry(
                            fd, u.VIDIOC_G_FMT, fmt, deadline, "current_format"
                        )
                        if reason is None:
                    ...
                    if reason is None:
                        ...
                        try:
                            buffer = bytearray(sizeimage)
                        except (MemoryError, OverflowError):
                            reason = "frame_buffer_allocation_failed"
                    if reason is None and buffer is not None:
                        poller = self._poll_factory()
                        poller.register(fd, select.POLLIN | select.POLLRDNORM)
                        reason, poll_attempted = self._poll_ready(poller, fd, deadline)
                    if reason is None and buffer is not None:
            ...
            try:
                ...
                closed = False
        if buffer is not None and not wiped:
        ...
        return CaptureFrameProbeResult(
            ...
            cap_ok,
            fmt_ok,
            "readwrite" if buffer is not None else None,
            poll_attempted,
            read_attempted,
            received,
            count,
            ...
            sizeimage,
            wiped,
            (buffer is None or wiped) and closed,
            False,
        )

    def _ioctl_with_retry(
        self, fd: int, request: int, buffer: bytearray, deadline: float, prefix: str
    ) -> tuple[str | None, bool]:
        attempts = 0
        while attempts < _MAX_IOCTLS:
            attempts += 1
            try:
                ...
                if error.errno == errno.EINTR:
                    if attempts < _MAX_IOCTLS and self._monotonic() < deadline:
                        continue
                    return f"{prefix}_ioctl_interrupted_budget_exhausted", False
    ...
    def _poll_ready(
        ...
        for attempt in range(_MAX_POLLS):
            try:
                events = poller.poll(max(0, int((deadline - self._monotonic()) * 1000)))
                if not events:
                ...
                if mask & (select.POLLIN | select.POLLRDNORM):
                    return None, True
                return "poll_unexpected_events", True
            except OSError as error:
                if (
    ...
    def _read(
        ...
        for attempt in range(_MAX_READS):
            try:
                ...
                if (
                    ...
                    continue
                if error.errno == errno.EINTR:
                    return "frame_read_interrupted_budget_exhausted", True, False, None
