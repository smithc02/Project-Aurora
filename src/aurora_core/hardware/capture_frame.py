...
from aurora_core.hardware.capture_frame_probe import CaptureFrameProbe
...
def _streaming_events(result: CaptureFrameProbeResult) -> tuple[bool, ...]:
    return (
        result.buffer_negotiation_succeeded,
        result.buffer_was_mapped,
        result.buffer_was_queued,
        result.stream_was_started,
        result.frame_dequeue_was_attempted,
        result.stream_was_stopped,
        result.buffer_was_unmapped,
        result.buffers_were_released,
    )


def _streaming_success_path_complete(result: CaptureFrameProbeResult) -> bool:
    return all(_streaming_events(result))
...
def _healthy_result(result: CaptureFrameProbeResult) -> bool:
    common = (
        ...
        and result.poll_was_attempted
    )
    if not common:
        return False

    if result.acquisition_method == "readwrite":
        return (
            result.frame_read_was_attempted
            and not result.streaming_io_was_used
            and not result.frame_dequeue_was_attempted
            and not any(_streaming_events(result))
        )

    if result.acquisition_method == "mmap":
        return (
            result.streaming_io_was_used
            and not result.frame_read_was_attempted
            and _streaming_success_path_complete(result)
        )
...
def _consistent_result(result: CaptureFrameProbeResult) -> bool:
    ...
    if method not in {None, "readwrite", "mmap"}:
        return False
    if result.streaming_io_was_used != (method == "mmap"):
        return False
    ...
    if result.frame_received and not _bounded_frame_metadata(result):
        return False

    if method == "readwrite":
        if any(_streaming_events(result)):
            return False
        if result.frame_dequeue_was_attempted:
            return False
        ...
        if result.frame_byte_count is not None and not result.frame_read_was_attempted:
            return False
        if result.frame_received and not result.frame_read_was_attempted:
            return False

        expected_cleanup = (
            result.descriptor_was_closed and result.frame_buffer_wipe_completed
        )

    elif method == "mmap":
        if result.frame_read_was_attempted:
            return False
        if result.buffer_was_mapped and not result.buffer_negotiation_succeeded:
            return False
        if result.buffer_was_queued and not result.buffer_was_mapped:
            return False
        if result.stream_was_started and not result.buffer_was_queued:
            return False
        if result.poll_was_attempted and not result.stream_was_started:
            return False
        if result.frame_dequeue_was_attempted and not (
            result.stream_was_started and result.poll_was_attempted
        ):
            return False
        if result.stream_was_stopped and not result.stream_was_started:
            return False
        if result.buffer_was_unmapped and not result.buffer_was_mapped:
            return False
        if result.buffers_were_released and not result.buffer_negotiation_succeeded:
            return False
        if result.frame_buffer_wipe_completed and not result.buffer_was_mapped:
            return False
        if (
            result.frame_byte_count is not None
            and not result.frame_dequeue_was_attempted
            ...
            return False
        if result.frame_received and not result.frame_dequeue_was_attempted:
            return False

        expected_cleanup = (
            result.descriptor_was_closed
            and (
                not result.buffer_negotiation_succeeded or result.buffers_were_released
            )
            and (
                not result.buffer_was_mapped
                or (result.frame_buffer_wipe_completed and result.buffer_was_unmapped)
            )
            and (not result.stream_was_started or result.stream_was_stopped)
        )

    else:
        expected_cleanup = result.descriptor_was_closed
    ...
    if result.frame_received:
        if result.reason_code in {
            ...
            "streaming_cleanup_failed",
        }:
            return not result.cleanup_completed
        if result.frame_buffer_wipe_completed:
            return False

    if method is not None and not result.frame_buffer_wipe_completed:
        if method == "readwrite" or result.buffer_was_mapped:
...
def validate_capture_frame(
    ...
    *,
    ...
    if not settings.capture_device.enabled:
        ...
        try:
            result = (probe or LinuxV4L2AdaptiveCaptureFrameProbe()).probe(
    ...
    return CaptureFrameValidationReport(
        ...
        buffer_negotiation_succeeded=result.buffer_negotiation_succeeded,
        buffer_was_mapped=result.buffer_was_mapped,
        buffer_was_queued=result.buffer_was_queued,
        stream_was_started=result.stream_was_started,
        frame_dequeue_was_attempted=result.frame_dequeue_was_attempted,
        stream_was_stopped=result.stream_was_stopped,
        buffer_was_unmapped=result.buffer_was_unmapped,
        buffers_were_released=result.buffers_were_released,
    )
