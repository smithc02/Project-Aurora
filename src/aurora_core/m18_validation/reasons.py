"""Finite normalized reason registry for synthetic Milestone 18 validation."""
...
def normalize_component_reason(
    *,
    ...
    """Map one exact current sanitized component projection to fixed reasons.

    ``message`` is deliberately unused. Ignored current detail values are never
    copied into the result, serialized, compared, or interpolated.
    """
    del message
    if type(schema_version) is not int or schema_version != 1:
        return _rejected(RejectionCode.UNKNOWN_SCHEMA)
    if component not in {"wled", "hyperhdr", "capture", "raspberry_pi"}:
...
def _normalize_wled(status: str, details: dict[str, object]) -> ReasonResult:
    fixed = _fixed_short_reason(status, details, "wled_disabled")
    ...
    if info not in _WLED_INFO_REASONS or state not in _WLED_STATE_REASONS:
        return _rejected(RejectionCode.UNKNOWN_VALUE)
    expected_status = _wled_status(str(info), str(state))
...
def _normalize_hyperhdr(status: str, details: dict[str, object]) -> ReasonResult:
    ...
    expected = "degraded" if reasons else "healthy"
    if status != expected:
        return _rejected(RejectionCode.INCONSISTENT_SNAPSHOT)
    return ReasonResult(
...
def _normalize_capture(status: str, details: dict[str, object]) -> ReasonResult:
    if details == {"reason_code": "collector_failed"}:
        if status != "unavailable":
            return _rejected(RejectionCode.INCONSISTENT_SNAPSHOT)
        return _accepted(NormalizedReason.CAPTURE_COLLECTOR_FAILED)
    keys = set(details)
...
def _normalize_pi(status: str, details: dict[str, object]) -> ReasonResult:
    if details == {"reason_code": "collector_failed"}:
        if status != "unavailable":
            return _rejected(RejectionCode.INCONSISTENT_SNAPSHOT)
        return _accepted(NormalizedReason.RASPBERRY_PI_COLLECTOR_FAILED)
...
def _fixed_short_reason(
    status: str, details: dict[str, object], disabled_value: str
) -> ReasonResult | None:
    if details == {"reason_code": "collector_failed"}:
        if status != "unavailable":
            return _rejected(RejectionCode.INCONSISTENT_SNAPSHOT)
        return _accepted(NormalizedReason.WLED_COLLECTOR_FAILED)
    if details == {"reason_code": disabled_value}:
        if status != "unavailable":
            return _rejected(RejectionCode.INCONSISTENT_SNAPSHOT)
        return _accepted(NormalizedReason.WLED_DISABLED)
    return None
