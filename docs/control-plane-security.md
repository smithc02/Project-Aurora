...
Milestone 14 adds an authenticated boundary for future Project Aurora controls.
It does not add a device control. The public health portal and `GET /api/health`
...
| Protected status | `GET /controls`, `GET /api/control/status` | Authenticated placeholder and a versioned capability response. No operations are registered. |
...
The protected capability response reports `mutations_enabled: false` and an
empty `available_operations` list. There is no generic execute route, WLED or
HyperHDR mutation, DDP output, system command, service action, configuration
write, capture operation, room-zone output, or AI operation.

Authentication and control-plane requests do not query the public health
snapshot. They cannot trigger WLED, HyperHDR, capture, or host polling. Public
...
Logout is the only authenticated state-changing form in this milestone. It
contains a strong per-session CSRF token and posts it in the request body, never
in a URL. Validation uses a constant-time comparison. A missing, malformed,
expired, or incorrect token rejects logout without invalidating a different
session. Every future authenticated mutation must reuse this requirement.
...
The `next` value is a fixed allowlist, currently only `/controls`. Absolute,
external, protocol-relative, and otherwise unknown destinations fall back to
that internal route.
...
Audit fields contain only schema version, event name, and bounded reason code.
...
## Future typed operation contracts

Milestone 14 defines non-executable metadata for later operations and registers
an empty operation tuple. It deliberately exposes no executor. A future
milestone must separately review and implement all of these boundaries before
...
6. Open `/login`, authenticate, and confirm that `/controls` reports mutations
   disabled and no available operations.
